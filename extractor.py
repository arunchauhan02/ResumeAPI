...
PROMPT_TEMPLATE = "You are an expert resume parser. Given the resume text, extract the fields Name , Email , Phone , LinkedIn , Skills , Education , Experience , Projects , Certifications , Languages and return a single valid JSON object. DO NOT include any introductory text,concluding remarks, or mark down code fence('''json). The Response must be a pure valid json object. Rules:- If a field cannot be found, set its value to No idea. - Return ONLY valid JSON (no extra commentary). - Keep lists as arrays, and keep Experience/Projects as arrays of short strings. Resume text: {text}"
